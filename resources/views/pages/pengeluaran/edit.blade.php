        ...
        <div class="col-xl-8">
            ...
            <div class="panel panel-inverse" data-sortable-id="form-plugins-3">
                ...
                <div class="panel-body p-0">
                    @if (isset($data))
                        <form class="form-horizontal form-bordered" action="{{ route('pengeluaran.update', $data->id) }}"
                            method="post" enctype="multipart/form-data">
                            @method('PUT')
                        @else
                            <form class="form-horizontal form-bordered" action="{{ route('pengeluaran.store') }}"
                                method="post" enctype="multipart/form-data">
                    @endif
                            ...
                            <div class="col-lg-8">
                                <select class="form-select" name="kegiatan_id" aria-label="Default select example" required>
                                    ...
                                    @foreach ($kegiatan as $item)
                                        <option value="{{ $item->id }}"
                                            {{ isset($data) && $data->kegiatan_id == $item->id ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-8">
                                <select class="form-select" name="kebutuhan_id" aria-label="Default select example"
                                    ...
                                    @foreach ($kebutuhan as $item)
                                        <option value="{{ $item->id }}"
                                            {{ isset($data) && $data->kebutuhan_id == $item->id ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-8">
                                <input type="text" class="form-control uang" name="saldo" id="rupiah"
                                    placeholder="Rp ....." required value="{{ $data->saldo }}">
                            </div>
                        </div>
